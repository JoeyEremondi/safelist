...
fromList (SafeList lengthSpec) unsafeList =
    if (List.length lengthSpec == List.length unsafeList) then
        (Just <| SafeList unsafeList)
    else
        Nothing
